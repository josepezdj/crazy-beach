...
export const playerService = (() => {
    ...
    services.getCurrentPlayer = () => {
        ...
        if (state !== -1 && state.currentPlayer !== '') return state.currentPlayer;
        return -1;
    }
    ...
    services.updateCurrentPlayer = (prop, value) => {
        ...
        if (state !== -1 && state.currentPlayer !== '') {
            ...
            if (!currentPlayer[prop] === value) {
                ...
                const foundPlayer = localstorageService.findPlayerInStatePlayers(state.currentPlayer.name)
                foundPlayer[prop] = value;
                const playerIndex = state.players.map(player => player.name).indexOf(foundPlayer.name);
    ...
    services.deleteCurrentPlayer = () => {
        ...
        if (state !== -1 && state.currentPlayer !== '') {
            ...
            state = state.players.filter(player => player.name !== state.currentPlayer.name);
            // Unset currentPlayer
            state.curentPlayer = '';
            // Save state
            localstorageService.saveStateToLocalStorage(state);
